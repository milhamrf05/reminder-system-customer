      ...
      <div class="row">
        ...
        <div class="col-md-6 col-xs-12 col-sm-10 q-mb-md">
          <q-input
            ...
            class="q-mr-sm"
            type="text"
            v-model="addReminderCustomerForm.customer_number_phone"
            lazy-rules
            label="Nomor HP Pelanggan *"
            :error="v$.customer_number_phone.$error"
            :error-message="
              v$.customer_number_phone.$errors.map((e) => e.$message).join()
            "
            @input="v$.customer_number_phone.$touch"
            @blur="v$.customer_number_phone.$touch"
          />
        </div>
        <div class="col-md-6 col-xs-12 col-sm-10 q-mb-md">
          <q-input
...
const rules = {
  ...
  customer_number_phone: { required: useRequired() },
  ...
  if (!v$.value.$invalid) {
    try {
      await useApiWithAuthorization.post('vehicle-service-records', {
        ...
        customer_phone_number: addReminderCustomerForm.customer_number_phone,
      });
