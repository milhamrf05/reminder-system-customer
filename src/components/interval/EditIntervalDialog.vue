<template>
  <q-dialog v-model="$state.showEditIntervalDialog">
      ...
      <q-form @submit="onSubmit">
        ...
        <q-card-actions align="right">
          <q-btn
            ...
            label="Cancel"
            color="primary"
            @click="$state.showEditIntervalDialog = false"
          />
...
const onSubmit = async () => {
  if (!v$.value.$invalid) {
    await useApiWithAuthorization.put(
      `intervals/${$state.editedInterval.id}`,
      intervalForm
    );
    $state.showEditIntervalDialog = false;
...
watchEffect(() => {
  intervalForm.name = $state.editedInterval.name;
});
</script>
